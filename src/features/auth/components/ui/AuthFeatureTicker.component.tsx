...
import { authFeatureTickerStyles } from '../../styles/auth/auth.styles';
...
export function AuthFeatureTicker({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div
      className={authFeatureTickerStyles.ticker}
      style={{
        width: '100%',
        overflow: 'hidden',
    ...
    >
      <div className={authFeatureTickerStyles.track} style={{ gap: compact ? 14 : 22 }}>
        {items.map((item, i) => (
          <span
            key={`${item.text}-${i}`}
            style={{
